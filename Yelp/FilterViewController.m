...
- (void) onApplyButton {
    NSDictionary * options = [NSDictionary dictionaryWithObjectsAndKeys:self.selectedSettings,
                              @"selectedSettings",
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    switch (section)
    {
        ...
        case 2:
            return 1;
        case 3:
            return self.showAllCategories ? self.categories.count : 5;
        default:
            return 0;
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.section)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            // special case: row 5 is "see all", if we're not showing all
            if (indexPath.row == 4 && ! self.showAllCategories)
            {
                // it'd be nice to get this middle-aligned
                cell.settingNameLabel.text = @"See all";
                cell.settingSwitch.hidden = YES;
            }
            else
            {
                cell.settingNameLabel.text = [[self.categories objectAtIndex:indexPath.row] valueForKey:@"name"];
    ...
    {
        cell.settingSwitch.on = [self.selectedCategories containsObject:[self.categories[indexPath.row] valueForKey:@"code"]];
    }
...
- (void)didChangeSwitch:(SettingCell *)settingCell {
    ...
    {
        NSLog(@"Need to get all the selected categories");

        if (settingCell.settingSwitch.on)
            [self.selectedCategories addObject:[self.categories[indexPath.row] valueForKey:@"code"]];
        else
            [self.selectedCategories removeObject:[self.categories[indexPath.row] valueForKey:@"code"]];
    }
...
- (void) tableView: (UITableView *) tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 3 && !self.showAllCategories && indexPath.row == 4)
    {
